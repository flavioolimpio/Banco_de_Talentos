# apps/inscricoes/management/commands/exportar_colaboradores_pesquisador_convocados.py
# Banco de Talentos — Polo de Inovação IFG
# Exporta um .xlsx com as inscrições de Colaborador(a) Externo(a) — Pesquisador
# aprovadas, contendo Nome, CPF, Titulação (nível de formação) e Pontuação
# validada pelo RH, ordenadas da maior para a menor pontuação — ordem de
# convocação.
#
...
#
# Uso:
#   python manage.py exportar_colaboradores_pesquisador_convocados
#   python manage.py exportar_colaboradores_pesquisador_convocados --saida x.xlsx
#
...
class Command(BaseCommand):
    help = (
        "Gera um .xlsx das inscrições de Colaborador(a) Externo(a) Pesquisador "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--saida",
            default=f"colaboradores_pesquisador_convocados_{timezone.localdate()}.xlsx",
            help=(
                "Caminho do arquivo .xlsx a gerar (padrão: "
                "colaboradores_pesquisador_convocados_<AAAA-MM-DD>.xlsx no diretório atual)."
            ),
        )
    ...
    def handle(self, *args, **options):
        aprovadas = (
            Inscricao.objects.filter(
                ...
                tipo_servidor=TipoServidor.PESQUISADOR,
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                f"Colaboradores Externos (Pesquisador) aprovados: {total}"
            )
        )

        if total == 0:
            self.stdout.write(
                "Nenhum colaborador externo Pesquisador aprovado — nada a exportar."
            )
            return
